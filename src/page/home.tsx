...
export default function Home() {
    ...
    try {
      ...
      setConnectedUsers(
        data.items
          ...
          .filter(({ id_usuario }) => id_usuario !== user!.id_usuario),
      );
    } catch (error) {
      console.error(error);
    }
  ...
  function MessageItem(item: RecentMessage) {
    const author =
      item.id_remetente === user?.id_usuario
    ...
    return (
      <TouchableOpacity style={styles.talk} onPress={onPress}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      <TouchableOpacity onPress={logout} style={styles.iconNameContainer}>
        <InitialIcon text={user!.nome} />
        <Text style={[styles.title, { marginLeft: 11 }]}>{user!.nome}</Text>
      </TouchableOpacity>
      <View>
        <Text style={styles.title}>
          Usuários conectados ({connectedUsers.length})
        </Text>
        <FlatList
      ...
      >
        <MaterialCommunityIcons name="forum" size={28} color="#FFF" />
      </TouchableOpacity>
    </SafeAreaView>
  );
}
