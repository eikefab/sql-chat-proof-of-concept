...
export default function Users() {
  ...
  async function fetch() {
    ...
    try {
      const { data } = await api.get<PaginatedItems<User>>("usuarios", {
        params: { p_nome: query },
      });

      setUsers(
        data.items.filter(({ id_usuario }) => user!.id_usuario !== id_usuario),
      );
    } catch (error) {
      console.error(error);
    }
  ...
  function UserItem(item: User) {
    return (
      <Pressable
        onPress={() =>
          navigation.navigate("Conversa", { targetId: item.id_usuario })
        }
        style={{
          alignItems: "center",
      ...
      >
        ...
        <Text style={{ marginTop: 5, width: 80, textAlign: "center" }}>
          {item.nome}
        </Text>
      </Pressable>
    );
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.contentContainer}>
        ...
        {previousQuery && (
          ...
          >
            Mostrando resultados para "{previousQuery}"
          </Text>
        )}
